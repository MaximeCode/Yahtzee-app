...
import Dice3D from "../Components/Dice3D"; // Le nouveau composant
...
export function Game() {
  ...
  //points 1ère partie
  const [pointsFirstGame, setpointsFirstGame] = useState(
    initialPointsFirstGame
  );
  //cases cliquées 1ère partie
  const [clickedCaseFG, setClickedCaseFG] = useState(initialClickedCaseFG);
  //points 2nde partie
  const [pointsSecondGame, setpointsSecondGame] = useState(
    initialPointsSecondGame
  );
  ...
  // NOUVEAUX ÉTATS POUR LES DÉS 3D
  const [is3DMode, setIs3DMode] = useState(true);
  const [shouldRoll3D, setShouldRoll3D] = useState(false);
  const [pendingDiceResults, setPendingDiceResults] = useState({});
  ...
  const rerollDice = () => {
    if (is3DMode) {
      // Mode 3D - déclencher le lancer 3D
      setShouldRoll3D(true);
      setLaps(laps - 1);
      setPendingDiceResults({});
      setDiceRollCount(0);
    } else {
      // Mode 2D - comportement original
  ...
  // Gestion des résultats des dés 3D
  const handleDice3DRoll = (diceIndex, result) => {
    // Ne traiter que les dés non gardés
    if (keptDice[diceIndex]) return;

    setPendingDiceResults((prev) => {
      const newResults = { ...prev, [diceIndex]: result };

      // Compter combien de dés non-gardés ont été lancés
      const nonKeptIndices = keptDice
        .map((kept, idx) => (kept ? null : idx))
        .filter((idx) => idx !== null);
      const resultsCount = Object.keys(newResults).length;

      // Si tous les dés non-gardés ont donné un résultat
      if (resultsCount >= nonKeptIndices.length) {
        // Mettre à jour les valeurs des dés
        const newDiceValues = diceValues.map((value, idx) =>
          newResults.hasOwnProperty(idx) ? newResults[idx] : value
        );
        setDiceValues(newDiceValues);
  ...
  const addPts = (index) => {
    ...
    setpointsFirstGame(
      pointsFirstGame.map((number, i) => (i === index ? result : number))
    );
  ...
  const upperTotal = (points) =>
    subtotal(points) + calculateBonus(points, firstGameIsFinished);
  const lowerTotal = (points) =>
    subtotal(points) + calculateBonus(points, firstGameIsFinished);
  ...
  const addScore = async () => {
    try {
      const response = await fetch(
        "https://yahtzee.app.albabadminton.fr/add_scores.php",
        {
          method: "POST",
          headers: {
    ...
    <div className="game pb-4">
      ...
      <div className="mb-3 d-flex justify-content-center">
        <button
          ...
          className="btnToClick fs-6"
          style={{ background: is3DMode ? "#266041" : "#8B4513" }}
        >
          {is3DMode ? "🎲 Passer aux dés 2D" : "🎯 Passer aux dés 3D"}
        </button>
      </div>
        ...
        {!firstGameIsFinished ? (
          countGame < 6 ? (
            ...
            <div className="col">
              ...
              {is3DMode ? (
                // Affichage des dés 3D
                <>
                  <Dice3D
                    diceValues={diceValues}
                    onDiceRoll={handleDice3DRoll}
                  ...
                  {diceValues[0] !== 0 && (
                    <>
                      <div className="row my-3" id="dice3DControls">
                      ...
                      <div className="my-md-3 fs-3 fw-bold">
                        Total of 5 dice : {total5Dice}
                      </div>
                    </>
                  )}
                </>
              ) : (
                // Affichage des dés 2D (original)
                diceValues[0] !== 0 && (
                  <>
                    ...
                    <div className="my-md-5 fs-3 fw-bold">
                      Total of 5 dice : {total5Dice}
                    </div>
                  </>
                )
              ...
              {laps >= 1 ? (
                <button
                  type="button"
                  onClick={rerollDice}
                  className="btnToClick my-4 fs-5"
                >
                  Throw the dice
                </button>
              ) : (
                <button
                  type="button"
                  disabled
                  className="btnToClick my-4 fs-5 grey cursor-disabled"
                >
                  It was the last lap
                </button>
              )}
            ...
            <div className="col my-5 fs-3 fw-bold textEndFirstGame">
              ...
              <div>
                Congratulations, you can now proceed to the 2nd and final part!
              </div>
              <div>🍀 Good luck 😉! 🍀</div>
              <button
                type="button"
            ...
            {is3DMode ? (
              ...
              diceValues[0] !== 0 && (
                <>
                  ...
                  <div className="my-5 fs-3 fw-bold">
                    Total of 5 dice : {total5Dice}
                  </div>
                </>
              )
          ...
          <div className="col mb-5 fs-3 fw-bold textEndFirstGame">
            ...
            >
              ...
              <Link
                to="/"
                className="btnToClick d-flex align-items-center text-decoration-none"
                id="backToHome"
                onClick={() => {
                  resetGame();
                }}
            ...
            <Modal show={show} onHide={handleClose} className="modals">
              ...
              <Modal.Body>
                <div className="mb-3 text-center">
                  <label htmlFor="player_name" className="col-form-label fs-4">
                    What's your name ?
                  </label>
                  <input
                    type="text"
          ...
          {firstGameIsFinished ? (
            countSecondGame < 7 ? (
              ...
              <>
                ...
                <div className="mt-3 d-flex justify-content-center flex-column flex-md-row flex-wrap gap-2">
                  <ScoreTable1
